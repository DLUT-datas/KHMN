...
The MS/MS similarity algorithm used here refers to gnpsalignment 
...
###
def score_alignment(spec1,spec2,pm1,pm2,tolerance,tolerance_MS2,max_charge_consideration=1):
    ...
    zero_shift_alignments = find_match_peaks_efficient(spec1_n,spec2_n,0,tolerance_MS2)
    real_shift_alignments = []
    if abs(shift) > tolerance:
        real_shift_alignments = find_match_peaks_efficient(spec1_n,spec2_n,shift,tolerance_MS2)

        if max_charge_consideration > 1:
            for charge_considered in range(2, max_charge_consideration + 1):
                real_shift_alignments += find_match_peaks_efficient(spec1_n,spec2_n,shift/charge_considered,tolerance_MS2)
